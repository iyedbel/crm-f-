...
<body class="bg-gray-50">
    ...
    <div class="container-box mx-auto py-8">
        ...
        <div class="flex justify-between items-center mb-4">
            <div>
                <label for="filterDate">Filtrer par date de création:</label>
                <input type="date" id="filterDate" class="px-4 py-2 border rounded-md">
            </div>
            <div>
            ...
            <table class="table-auto w-full bg-white shadow-lg rounded-lg overflow-hidden">
                <thead class="bg-blue-200">
                    <tr>
                        ...
                        <th class="px-4 py-2 date-creation-column">Date de création</th>
                ...
                <tbody>
                    ...
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<td class='px-4 py-2 text-center date-creation-column'>" . $row["date_creation"] . "</td>";
                            echo "<td class='px-4 py-2 text-center'>";
                            echo '<div class="flex justify-center">'; // Center buttons
                            ...
                            echo '<a class="btn btn-danger py-1 px-2 rounded-md bg-red-500 text-white hover:bg-red-600" href="/CRM/deletevend.php?id=' . $row["id"] . '">Supprimer</a>';
                            echo '</div>';
                            echo "</td>";
                            echo "</tr>";
                        }
                    } else {
                        echo "<tr><td colspan='9' class='text-center'>Aucun vendeur trouvé</td></tr>";
                    }
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            function applyFilters() {
                ...
                rows.forEach(row => {
                    const dateCreation = row.querySelector(".date-creation-column").textContent.trim();
                    const wilaya = row.querySelector(".wilaya-column").textContent.trim().toLowerCase();
                    const dateMatch = !selectedDate || dateCreation === selectedDate;
                    const wilayaMatch = !selectedWilaya || wilaya === selectedWilaya;

                    if (dateMatch && wilayaMatch) {
                        row.style.display = "";
                    } else {
