...
.box {
    ...
    display: flex; /* Add this to center text vertically */
    ...
    <div class="overflow-x-auto mt-8">
        <table class="table-auto w-full bg-white shadow-lg rounded-lg overflow-hidden">
            ...
            <tr>
                    ...
                    <th class="px-4 py-2">id_vendeur</th>
                    ...
                    <th class="px-4 py-2">Prix_totale</th>
...
$result = $conn->query($sql);
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<td class='px-4 py-2 text-center'>" . $row["id_vendeur"] . "</td>";
                        ...
                        echo "<td class='px-4 py-2 text-center '>" . $row["prix_vente"] . "DA</td>";
                        echo "<td class='px-4 py-2 text-center'>" . $row["prix_livraison"] . " DA</td>";
                        ...
                        if ($row["statut"] == "Au bureau" || $row["statut"] == "En Sortie") {
                            echo "<td class='optionColumn px-4 py-2 text-center'>" . ($row["prix_vente"] + $row["prix_livraison"]) . " DA</td>"; // Prix total
                        } else {
                            echo "<td class='optionColumn px-4 py-2 text-center'>null</td>"; // Prix total
                        }
                        echo "<td class='px-4 py-2 text-center'>"; // Parameter button with icon
 ...
 // Query to fetch colis data
 $sql = "SELECT SUM(prix_vente + prix_livraison) AS total_price FROM colis WHERE statut IN ('Au bureau', 'En Sortie')";
 $result = $conn->query($sql);

 // Récupérer la somme des prix totaux
 $total_price = 0;
 if ($result->num_rows > 0) {
 ...
 // Afficher la somme des prix totaux dans la boîte
 echo "<div class='container'>";
 echo "<div class='box' id='box1'>";
 echo "<p>Total: " . $total_price . " DA</p>";
 echo "</div>";
 echo "</div>";
        ...
        <span class="absolute top-0 right-0 p-2 cursor-pointer" id="closeModal">
          <i class="fas fa-times text-gray-600"></i> <!-- Vous pouvez remplacer "fas fa-times" par l'icône de votre choix -->
            ...
            <form id="statusForm" method="POST">
                ...
                <div class="mb-4">
                    <label for="statut" class="block text-gray-700">Nouveau statut:</label>
                 <select name="statut" id="statut" class="w-full px-4 py-2 border border-gray-300 rounded-md">
                    ...
                    <option value="Distinataire refusé">Destinataire refusé</option>
                    <option value="Retour à l'expéditeur">Retour à l'expéditeur</option>
                </select>
